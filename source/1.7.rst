...
Paddle Lite 是飞桨深度学习框架的一款轻量级、低框架开销的推理引擎，除了在移动端侧应用外，您还可以使用服务器进行 Paddle Lite 预测。Paddle 预测采用子图的形式集成 Paddle Lite，以方便用户在使用 Paddle 原有方式上稍加改动，即可开启 Paddle Lite 的预测能力，得到更快的预测速度。将来，使用 Paddle Lite 将可以在昆仑等百度自研芯片上执行预测。
