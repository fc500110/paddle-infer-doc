...
Paddle Lite是飞浆团队开发的一款轻量级推理引擎，支持多种场景下的轻量高效预测，是一个高性能、轻量级的深度学习引擎。
与集成TensorRT类似，Paddle以子图的形式集成了Lite。针对一些计算量较小，实际预测耗时很少的小模型，如果直接使用Paddle Inference预测，框架耗时可能与模型耗时在同一量级，此时可选用以Paddle Lite子图的方式来运行。

在已经配置预测的基础上, 只需再添加一行配置就可以开启 Paddle-Lite 加速预测：

.. code:: python

    config.enable_lite_engine(precision_mode=AnalysisConfig.Precision.Float32)

Paddle Lite十分注重性能，从框架层面到底层算子都进行了全方位的优化，简要描述如下：
...
* Kernel优化
在 Kernel 层面，对相应硬件上的 Kernel 通过指令集、算法改写等方式进行了深度的优化。如在x86上通过jit实现多个常用算子、在NVIDIA GPU上通过对算子的计算过程进行分析，改写算法，对多次的矩阵乘法进行融合等操作，提升kernel的运行效率。
...
* 多硬件混合调度
Lite 支持系统可见任意硬件的混合调度，已验证了 X86 CPU 和 Nvidia GPU 间的混合调度。混合调度的支持，使得当系统内同时存在多种硬件可用时，Lite可以充分利用各类硬件资源，从而提升性能。
